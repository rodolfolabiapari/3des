...
void upper_char(char * input) {
	...
	for (i = 0; i < length; i++) {
		...
		if (input[i] > 96 && input[i] < 123) input[i] -= 32;
		//printf("%d\n", input[i]);
	}
}
...
void hex_to_plain(char in[][16] , unsigned char * out, int t) {
	int i, j = 0, k, z, sum;
	char temp[3];
	...
	for (k = 0; k < t; k++) {
		...
		for (i = 0; i < 16; i = i + 2) {
			...
			if (temp[0] >= 65 && temp[0] <= 71) z = switch_case(temp[0]);
			...
			temp[1] = in [k][i + 1];
			//Verifi se é alguma letra e retorna seu número em decimal
			if (temp[1] >= 65 && temp[1] <= 71) z = switch_case(temp[1]);
			else z = temp[1] - 48;
...
/*//Procedimento que transforma um texto Hexadecimal em um texto legível ao usuário
void hex_to_plain(int linhas, char in[linhas][16] , char * out, int t) {
	int i, j = 0, k, z, sum;
	char temp[3];
...
void copyResultado(int linhas, char in[linhas][16], char * out) {
	int i, j;
	for (i = 0; i < linhas; i++) 
		for (j = 0; j < 16; j++)
		* (out + (i * 16) + j) = in[i][j];
...
void make_key(char * a, char * b, char * c) {
	...
	for (i = 0; i < 28; i++) * (c + i) = * (a + i);

	//Continua com a segunda sequência de caracteres
	for (i = 28; i < 56; i++) * (c + i) = * (b + (i - 28));
}
...
void arquivoResposta (char * r, char * nomeSaida, FILE * orienta) {
	FILE * saida;
...
/*
 void des_round_decry(char * Ri, char * Li, char * Ri1, char * Li1, char * ERi1, char * K1, char * F1) {
	char tp[5], f[32];
	short temp, i, row, column, j, limit = 0;
	copy(Ri1, Li);

	permut_48(Ri, ERi1);
	printf("\nPermutação 48. Ri1 to ERi1: ");
	for (i = 0; i < 48; i++) { printf("%c", ERi1[i]); if ((i + 1) % 8 == 0) {
			printf(" ");
		}
	}
	...
	xor(K1, ERi1, F1);
	printf("\nK1 Xor ER0 = F1		 : ");
	for (i = 0; i < 48; i++) { printf("%c", F1[i]); if ((i + 1) % 8 == 0) {
			printf(" ");
		}
	}
	...
	for (i = 0; i < 32; i++) {		printf("%c", Ri[i]);		if ((i + 1) % 8 == 0) {
			printf(" ");
		}
	}
...
void xor(char * K, char * ER, char * F) {
	int i, m = 0;
	for (i = 0; i < 48; i++) {
		if (( * (K + i) == '1' && * (ER + i) == '1') || ( * (K + i) == '0' && * (ER + i) == '0')) { * (F + m) = '0';
			m++;
		} else { * (F + m) = '1';
			m++;
		}
...
void xor_32(char * L0, char * f, char * R1) {
	short i, m = 0;
	for (i = 0; i < 32; i++) {
		if (( * (L0 + i) == '1' && * (f + i) == '1') || ( * (L0 + i) == '0' && * (f + i) == '0')) { * (R1 + m) = '0';
			m++;
		} else { * (R1 + m) = '1';
			m++;
		}
