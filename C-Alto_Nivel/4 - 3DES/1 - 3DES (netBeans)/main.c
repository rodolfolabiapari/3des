...
void leArquivo(FILE * arq, char * input, long size) {
...
void leChave(char * nomeArquivo, char keys[2][16], FILE * arq) {
	...
	/*if (feof(entrada) && j < 3) {
		printf("\n\nErro ao ler arquivo de entrada da senha!!!\n");
		fprintf(arq, "\n\nErro ao ler arquivo de entrada da senha!!!\n");
		exit(1);
	}*/
...
int main (int argc, char *argv[]) {
	...
	if (acao == 'e') {
		...
		textoSaida = "out.txt";
			if ((orienta = fopen("RelatorioDeExecucaoEncripta.txt", "w")) == NULL) {
		...
		if ((orienta = fopen("RelatorioDeExecucaoDesencripta.txt", "w")) == NULL) {
			printf("Erro ao criar arquivo de orientações!!!\n");
			exit(1);
		}
	}
	...
	char input[tamArquivo + 1], initial_hex[tamArquivo * 2 + 1], output[tamArquivo * 2 + 1];
	...
	if (acao == 'e' || acao == 'E') {
		...
		/*/chave(C, D, CD, K, orienta, textoChave, 1);
		for (i = 0; i < d * 2; i++) {	
			DES (manipulaTexto1, i, manipulaTexto2, orienta, K);
			printf("\n\n\n\n\n");
			fprintf(orienta, "\n\n\n\n\n");
	...
	printf ("\n\n>Decrypted manipulaTexto1: "); 
	printf ("%s\n\n",manipulaTexto1);

	 * 


	DES (manipulaTexto1, temp * 16, manipulaTexto2);
	printf ("\n\n>Decrypted manipulaTexto2: "); 
	printf ("%s\n\n",manipulaTexto2);

	DES (manipulaTexto2, temp * 16, crypto3);
	printf ("\n\n>Decrypted CRYPTO3: "); 
	printf ("%s\n\n",crypto3);
