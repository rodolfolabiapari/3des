...
void DES (char entrada[26][16], char key_bin[64], int indice, char cryptoOut[400], FILE * arquivo)
{
	...
	fprintf(arquivo, "\n\n\n\n\tSTARTING\n"); 
        ...
        printf("\n\n\n");
        printf("    INITIAL PERMUTATION PLAIN TEXT\n");
        ...
        fprintf(arquivo, "\n\n\n    16 ROUNDS\n");  
        //Uma iteração
        des_round (arquivo, L1,R1,L0,R0,ER0,K1,F1); 
        ...
        des_round (arquivo, L2,R2,L1,R1,ER1,K2,F2); 

        printLiRi(L2, R2, '2', arquivo);



        des_round (arquivo, L3,R3,L2,R2,ER2,K3,F3); 

        printLiRi(L3, R3, '3', arquivo);



        des_round (arquivo, L4,R4,L3,R3,ER3,K4,F4); 

        printLiRi(L4, R4, '4', arquivo);



        des_round (arquivo, L5,R5,L4,R4,ER4,K5,F5); 

        printLiRi(L5, R5, '5', arquivo);


        des_round (arquivo, L6,R6,L5,R5,ER5,K6,F6); 

        printLiRi(L6, R6, '6', arquivo);



        des_round (arquivo, L7,R7,L6,R6,ER6,K7,F7); 

        printLiRi(L7, R7, '7', arquivo);


        des_round (arquivo, L8,R8,L7,R7,ER7,K8,F8); 

        printLiRi(L8, R8, '8', arquivo);



        des_round (arquivo, L9,R9,L8,R8,ER8,K9,F9); 

        printLiRi(L9, R9, '9', arquivo);


        des_round (arquivo, L10,R10,L9,R9,ER9,K10,F10); 

        printLiRi(L10, R10, 'A', arquivo);


        des_round (arquivo, L11,R11,L10,R10,ER10,K11,F11); 

        printLiRi(L11, R11, 'B', arquivo);



        des_round (arquivo, L12,R12,L11,R11,ER11,K12,F12); 

        printLiRi(L12, R12, 'C', arquivo);


        des_round (arquivo, L13,R13,L12,R12,ER12,K13,F13);

        printLiRi(L13, R13, 'D', arquivo);



        des_round (arquivo, L14,R14,L13,R13,ER13,K14,F14); 

        printLiRi(L14, R14, 'E', arquivo);



        des_round (arquivo, L15,R15,L14,R14,ER14,K15,F15); 

        printLiRi(L15, R15, 'F', arquivo);



        des_round (arquivo, L16,R16,L15,R15,ER15,K16,F16); 
        ...
        fprintf (arquivo, "Key in Hexadecimal used for encryption: "); 
        for (i = 0; i < 16; i++) {
                printf ("%c",key_hex[i]);
                fprintf (arquivo, "%c",key_hex[i]); 
        }
        printf ("\n\n\n");
        fprintf (arquivo, "\n\n\n");*/
