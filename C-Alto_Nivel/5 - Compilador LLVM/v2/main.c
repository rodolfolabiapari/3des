...
void DES (unsigned char entrada[][16], int indice, unsigned char cryptoOut[][16], FILE * arq, char K[15][48]) {
	
	int i = 0; 
	
	char input_hex[16],input_bin[64], *encryption, encrypted[64],encry_permut[64];
	
	char L0[32], R0[32], ER0[48],
		 L1[32], R1[32], ER1[48], F1[48],
...
void UnDES (unsigned char entrada[][16], int indice, unsigned char cryptoOut[][16], FILE * arq, char K[15][48]){

	int i = 0; 
	
	char input_hex[16],input_bin[64], *decryption, decryption_final_hex[4000], decrypted[64],decry_permut[64]; 
	int q = -1;
...
int main (int argc, char *argv[]) {
	...
	fprintf (orienta, " -- Fim da execução do algoritmo." ); 
